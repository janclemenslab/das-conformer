...
class NPYDirDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str):
        self.gen = {}
        for which in self.data.keys():
            self.gen[which] = SyllableGenerator(
                time_steps=self.time_steps,
                specs=self.data[which]["x_spec"].T,
                labels=self.data[which]["y_spec"],
                training=False,
            )

    def _dataloader(self, which):
        return DataLoader(
            self.gen[which],
    ...
    def predict_dataloader(self):
        return None  # DataLoader(self.mnist_predict, batch_size=self.batch_size)
