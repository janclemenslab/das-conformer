...
class RawAudioFileGenerator(Dataset):
    def __init__(
        self,
        time_steps: int,
        audio_file: str,
...
class RawAudioFileDataModule(L.LightningDataModule):
    def __init__(
        self,
        audio_file: str,
        batch_size: int = 32,
        ...
        self.annotations = None
        soundfile.info(self.audio_file)
        annot_file = self.audio_file.parent / Path(str(self.audio_file.stem) + "_annotations.csv")
    ...
    def _dataloader(self, stage: str):
        self.gen = RawAudioFileGenerator(
