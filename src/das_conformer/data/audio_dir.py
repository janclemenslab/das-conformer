...
class AudioDirSyllableGenerator(Dataset):
    def __init__(
        self,
        time_steps: int,
        ...
        for nb_samples, samplerate in zip(self.nb_samples_in_file, self.samplerate_per_file):
            chunk_duration = self.time_steps * samplerate * self.hop_s
        ...
        self.nb_total = int(self.repeats * np.sum(self.nb_chunks_in_file))
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            id = np.random.choice(self.labels.shape[0] - self.time_steps - 1, p=self.file_prob)
            start = np.random.choice(self.nb_samples_in_file[id] - self.time_steps - 1)
        else:
        ...
        a.seek(start)
        chunk = a.read(frames=int((self.time_steps - 1) * hop_samples))
        ...
        S = np.log2(0.001 + S, dtype=np.float32)
        ...
        labels = np.zeros((self.time_steps, self.nb_classes), dtype=np.float32)
        if len(stops) and len(starts):
            ...
            starts = (starts * a.samplerate).astype(np.intp)
            stops = (stops * a.samplerate).astype(np.intp)
